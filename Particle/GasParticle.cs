...
namespace JourneyRecipes.Particle
{
    public class GasParticle : ABasicParticle
    {
        ...
        public override void Update(ref ParticleRendererSettings settings)
        {
            ...
            Scale = Vector2.One * InitialScale * JRUtils.Remap(fromValue, 0f, 0.95f, 1f, 1.3f);
            Opacity = MathHelper.Clamp(JRUtils.Remap(fromValue, 0f, FadeInNormalizedTime, 0f, 1f) * JRUtils.Remap(fromValue, FadeOutNormalizedTime, 1f, 1f, 0f), 0f, 1f) * 0.85f;
        ...
        public override void Draw(ref ParticleRendererSettings settings, SpriteBatch spritebatch)
        {
            ...
            Texture2D value = ModContent.GetTexture("Resprite/Projectile_511");
            Vector2 origin = new Vector2(value.Width / 2, value.Height / 2);
            Vector2 position = settings.AnchorPosition + LocalPosition;
            Color color = Color.Lerp(Lighting.GetColor(LocalPosition.ToTileCoordinates().X, LocalPosition.ToTileCoordinates().Y), ColorTint, 0.2f) * Opacity;
            Vector2 scale = Scale;
