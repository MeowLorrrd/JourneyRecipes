...
namespace JourneyRecipes.Items
{
    ...
    {
        public override bool Autoload(ref string name) => return GetInstance<JourneyRecipesServerConfig>().allowAccessoryStat;
        ...
        public override void SetDefaults(Item item)
        {
            ...
            else if (item.type == 3016)
            {
                item.defense = 8;
            }
            else if (item.type== 3245)
            {
                item.accessory = true;
                item.damage = 0;
                item.knockBack = 0;
